...
/**
 * Synchronous Stage Functions
 * 
 * These versions execute without jobId or database persistence.
 * Used for synchronous single-request audit execution.
 */

/**
 * Stage 1: Fast Pass - Basic audit data (Synchronous)
 */
export async function processStage1Sync(url: string): Promise<Partial<AuditResults>> {
  ...
  try {
    ...
    // Invalid URL - return empty partial audit
    return {
      url,
      finalUrl: targetUrl,
      status: 0,
...
/**
 * Stage 2: Structure + Media - Deeper checks (Synchronous)
 */
...
/**
 * Stage 3: AI Optimization - Heavier NLP analysis (Synchronous)
 */
export async function processStage3Sync(
  ...
  try {
    ...
    const results: Partial<AuditResults> = {
      ...
      aiMetrics,
      aiOptimizationTimeout: aiTimeout,
      partialAudit: aiTimeout || stage2Results.partialAudit, // Set partial_audit if AI stage timed out or if previous stage was partial
    };
